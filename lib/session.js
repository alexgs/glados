...
type GladosRequest = $Request & {
    ...
    user:?mixed         // TODO The user object can be expanded/better defined
};
...
function handleAnonymousSession( request:$Request, response:$Response, jwtToken:JwtToken ):Promise<AnonymousSessionData> {
...
function startAnonymousSession( request:$Request, response:$Response ):Promise<void> {
    const sessionCookie = getAnonSessionName();
    return new Promise( ( resolve, reject ) => {
        if ( request.cookies && request.cookies[ sessionCookie ] ) {
            // TODO --> Test this! <--
