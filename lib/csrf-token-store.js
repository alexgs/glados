...
function initialize( context ) {
    if ( !_.isObject( context ) ) {
        throw new Error( messageFactory.contextMustBeObject( context ) );
    }
    if ( store.size > 0 ) {
...
export const messageFactory = {
    contextMustBeObject: ( badContext ) => `The context argument must be an object, but its type is ${typeof badContext}.`,
