...
function completeOAuth2( publicKey:Buffer|string ) {
    ...
    return function( request:$Request, response:$Response, next:NextFunction ) {
        ...
        if ( typeof request.query.state === 'string' && !csrf.verifyToken( request.query.state ) ) {
        ...
        superagent.post( middlewareOptions.tokenUrl )
            ...
            .then( jwtToken => {
                ...
                return jwt.validateClaims( jwtToken, middlewareOptions.domain, middlewareOptions.clientId )
            } )
...
function startOAuth2() {
    ...
    return function( request:$Request, response:$Response ) {
        ...
        session.handleAnonymousSession( request, response )
            .then( () => {
