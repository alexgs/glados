...
interface CookieCrypto {
    encrypt( message:Buffer, nonce:Buffer, key:Buffer ):void;
    decrypt( cipher:Buffer, nonce:Buffer, key:Buffer ):void;
    key():Buffer;
    nonce():Buffer;
    KEYBYTES:number;
    MACBYTES:number;
...
function setAnonSessionCookie( response:$Response, cookiePayload:CookiePayload ) {
    ...
    const payloadString = ( cookiePayload instanceof String ) ? cookiePayload : JSON.stringify( cookiePayload );
    const payloadBuffer = Buffer.from( payloadString );
    const nonce = crypto.nonce();
    const cipherPayload = crypto.encrypt( payloadBuffer, nonce, key );
}
